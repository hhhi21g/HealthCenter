...
def fetch_page(url, browser_page):
    """使用真浏览器获取页面内容，专门对付 468 JS 质询"""
    try:
        logger.info(f"使用真浏览器请求页面: {url}")

        # 让真实的浏览器去访问这个网址
        browser_page.get(url)

        # 稍微等一等，让浏览器有时间处理安全验证和加载内容
        time.sleep(random.uniform(2.5, 4.0))

        html = browser_page.html

        # 简单的失败检测，如果依然有盾，多等几秒让浏览器自动刷新计算
        if len(html) < 1000 or "468" in html or "请稍候" in html:
            logger.warning("页面似乎仍在验证中，等待 5 秒...")
            time.sleep(5)
            html = browser_page.html

        logger.info(f"请求成功: 获取到网页源码，大小 {len(html)} 字节")
        return html, browser_page

    except Exception as e:
        logger.error(f"请求页面失败: {str(e)}")
        return None, browser_page
...
def crawl_jkzx_articles(max_pages=60):
    """爬取健康资讯网通知公告栏目文章 - 支持多页爬取"""
    base_url_pattern = "http://jkzx.org.cn/list-tongzhigonggao-{}.html"
    logger.info(f"开始爬取健康资讯网通知公告栏目，最多爬取 {max_pages} 页")

    # 🚀 初始化 DrissionPage 浏览器
    co = ChromiumOptions()
    # 调试阶段可以把下面这行注释掉，这样能亲眼看到浏览器弹出来；跑通后如果不想浏览器弹出来，就取消注释
    # co.headless()
    ...
    # 爬取多个页面（从第1页开始）
    for page_num in range(50, max_pages + 1):
        try:
            # 构建当前页URL
            current_page_url = base_url_pattern.format(page_num)
            logger.info(f"开始爬取第 {page_num}/{max_pages} 页: {current_page_url}")
            ...
            # 获取列表页
            list_html, _ = fetch_page(current_page_url, browser_page)
            if not list_html:
                logger.error(f"获取第 {page_num} 页失败，跳过")
                continue

            # 保存列表页用于调试
            debug_filename = f"jkzx_debug/list_page_{page_num}_{datetime.now().strftime('%Y%m%d%H%M%S')}.html"
            ...
            logger.info(f"第 {page_num} 页找到 {len(articles)} 篇文章")
            ...
            # 如果没有文章了，提前结束
            if not articles:
                logger.info(f"第 {page_num} 页没有文章，停止爬取")
                break

        except Exception as e:
            logger.error(f"爬取第 {page_num} 页时出错: {str(e)}")
            continue

    if not all_articles:
        logger.error("未找到任何文章，终止爬取")
        browser_page.quit()  # 找不到文章时也要关闭浏览器
        return []
    ...
    for idx, article in enumerate(all_articles):
        try:
            ...
            # 获取文章详情页
            detail_html, _ = fetch_page(article['url'], browser_page)
            if not detail_html:
...
if __name__ == "__main__":
    logger.info("健康资讯网通知公告栏目爬虫已启动，每小时运行一次.")
